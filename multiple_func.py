...
def func_reg_DOP(allvars, L, par):
    ...
    for i in range(len(crds1)):
        x = np.sin(crds1[i]) * np.cos(crds2[i])
        y = np.sin(crds1[i]) * np.sin(crds2[i])
        z = np.cos(crds1[i])

        X = x / (1 - z)
        Y = y / (1 - z)

        x_lua = np.sin(cLUA1[i]) * np.cos(cLUA2[i])
        y_lua = np.sin(cLUA1[i]) * np.sin(cLUA2[i])
        z_lua = np.cos(cLUA1[i])
        ...
        # epsilon+ definição
        Lepsip[0] = (X_lua + L * np.cos(cLUA2[i])) / (1 - L * np.cos(cLUA2[i]) * X_lua)
        Lepsip[1] = (Y_lua + L * np.sin(cLUA2[i])) / (1 - L * np.sin(cLUA2[i]) * Y_lua)
        LEp = Lepsip[0] + 1j * Lepsip[1]
        # outra representação complexa
        # LEp = 1j * (y_lua + L ) / (1 - L * y_lua)

        # -1/ (conjugado do epsilon+)
        LEpp = -1 / np.conj(LEp)
        # Lepsipp = np.zeros(2)
        # Lepsipp = xy(LEpp)

        # epsilon- definição
        Lepsil[0] = (X_lua - L * np.cos(cLUA2[i])) / (1 + L * np.cos(cLUA2[i]) * X_lua)
        Lepsil[1] = (Y_lua - L * np.sin(cLUA2[i])) / (1 + L * np.sin(cLUA2[i]) * Y_lua)
        LEl = Lepsil[0] + 1j * Lepsil[1]
        # outra representação complexa
        # LEl = 1j * (y_lua - L ) / (1 + L * y_lua)
...
def func_simple_reg_DOP(theta_field, phi_field, theta_lua, phi_lua, L, par):
    np.seterr(invalid='ignore')

    x = np.sin(theta_field) * np.cos(phi_field)
    y = np.sin(theta_field) * np.sin(phi_field)
    z = np.cos(theta_field)

    X = x / (1 - z)
    Y = y / (1 - z)

    x_lua = np.sin(theta_lua) * np.cos(phi_lua)
    y_lua = np.sin(theta_lua) * np.sin(phi_lua)
    z_lua = np.cos(theta_lua)
    ...
    # Lua
    # epsilon+ definição
    Lepsip_a = (X_lua + L * np.cos(phi_lua)) / (1 - L * np.cos(phi_lua) * X_lua)
    Lepsip_b = (Y_lua + L * np.sin(phi_lua)) / (1 - L * np.sin(phi_lua) * Y_lua)
    LEp = Lepsip_a + 1j * Lepsip_b

    # -1/ (conjugado do epsilon+)
    LEpp = -1 / np.conj(LEp)

    # epsilon- definição
    Lepsil_a = (X_lua - L * np.cos(phi_lua)) / (1 + L * np.cos(phi_lua) * X_lua)
    Lepsil_b = (Y_lua - L * np.sin(phi_lua)) / (1 + L * np.sin(phi_lua) * Y_lua)
    LEl = Lepsil_a + 1j * Lepsil_b
...
def func_hor_DOP(allvars, L, N):
    ...
    for i in range(len(crds1)):
        # gamma = val.func_gamma(crds1[i], crds2[i], cLUA1[i], cLUA2[i])
        ...
        wl = -1.0 * ((xyo - LEp) * (xyo - LEl) * (xyo - LEpp) * (xyo - LEll)) / (
                ((1 + (np.abs(xyo) ** 2)) ** 2) * np.abs(LEp - LEpp) * np.abs(LEl - LEll))

        DOP[i] = np.abs(wl) * np.cos(cLUA1[i]) ** (1 / N) * par_wave[i]

    return DOP


def func_simple_hor_DOP(theta_field, phi_field, theta_lua, phi_lua, par_wave, L, N):
    # gamma = val.func_gamma(crds1[i], crds2[i], cLUA1[i], cLUA2[i])
    ...
    DOP = np.abs(wl) * par_wave * np.cos(theta_lua) ** (1 / N)

    return DOP


def func_simple_hor(theta_field, phi_field, theta_lua, phi_lua, L, N):
    # gamma = val.func_gamma(crds1[i], crds2[i], cLUA1[i], cLUA2[i])
    ...
    DOP = np.abs(wl) * np.cos(theta_lua) ** (1 / N)
...
def func_turb_DOP(allvars, L, k, p):
    ...
    for i in range(len(crds1)):
        ...
        DOP[i] = np.abs(wl) * np.exp(-3 / k + p) * par_int[i]
...
def func_seeing_DOP(allvars, L, k, p):
    ...
    for i in range(len(crds1)):
        ...
        DOP[i] = np.abs(wl) * np.exp(-seeing[i] / k + p) * par_wave[i]

    return DOP


def func_simple_seeing_DOP(theta_field, phi_field, theta_lua, phi_lua, seeing, par_wave, L, k, p):
    x = np.sin(theta_field) * np.cos(phi_field)
    y = np.sin(theta_field) * np.sin(phi_field)
    z = np.cos(theta_field)
    ...
    DOP = np.abs(wl) * np.exp(-seeing / k + p) * par_wave

    return DOP


def func_simple_seeing(theta_field, phi_field, theta_lua, phi_lua, seeing, L, k, p):
    # gamma = val.func_gamma(crds1[i], crds2[i], cLUA1[i], cLUA2[i])
    ...
    DOP = np.abs(wl) * np.exp(-seeing / k + p)
...
def func_mix_DOP(allvars, L, N, k, p):
    ...
    for i in range(len(crds1)):
        ...
        DOP[i] = np.abs(wl) * np.cos(cLUA1[i]) ** (1 / N) * np.exp(-seeing[i] / k + p)

    return DOP


def func_simple_mix(theta_field, phi_field, theta_lua, phi_lua, seeing, L, N, k, d):
    # gamma = val.func_gamma(crds1[i], crds2[i], cLUA1[i], cLUA2[i])
    ...
    DOP = np.abs(wl) * np.cos(theta_lua) ** (1 / N) * np.exp(-seeing / k + d)
...
def func_simple_sun_DOP(theta_field, phi_field, theta_lua, phi_lua, theta_sol, phi_sol, alb, par_wave, L):
    global W
    ...
    x_sol = np.sin(theta_sol) * np.cos(phi_sol)
    y_sol = np.sin(theta_sol) * np.sin(phi_sol)
    z_sol = np.cos(theta_sol)
    ...
    # Sol
    # epsilon+ definição
    Sepsip_a = (X_sol + L * np.cos(phi_sol)) / (1 - L * np.cos(phi_sol) * X_sol)
    Sepsip_b = (Y_sol + L * np.sin(phi_sol)) / (1 - L * np.sin(phi_sol) * Y_sol)
    SEp = Sepsip_a + 1j * Sepsip_b
    # -1/ (conjugado do epsilon+)
    SEpp = -1 / np.conj(SEp)

    # epsilon- definição
    Sepsil_a = (X_sol - L * np.cos(phi_sol)) / (1 + L * np.cos(phi_sol) * X_sol)
    Sepsil_b = (Y_sol - L * np.sin(phi_sol)) / (1 + L * np.sin(phi_sol) * Y_sol)
    SEl = Sepsil_a + 1j * Sepsil_b
    # -1/ (conjugado do epsilon+)
    SEll = -1 / np.conj(SEl)
    ...
    wl = ((xyo - LEp) * (xyo - LEl) * (xyo - LEpp) * (xyo - LEll)) / (
            ((1 + (np.abs(xyo) ** 2)) ** 2) * np.abs(LEp - LEpp) * np.abs(LEl - LEll)) * par_wave

    ws = ((xyo - SEp) * (xyo - SEl) * (xyo - SEpp) * (xyo - SEll)) / (
            ((1 + (np.abs(xyo) ** 2)) ** 2) * np.abs(SEp - SEpp) * np.abs(LEl - SEll)) * par_wave

    if (90 - 180 / np.pi * theta_sol < -18).any():
        W = np.abs(wl) * alb
    if (90 - 180 / np.pi * theta_sol >= -18).any():
        W = np.abs(ws) + np.abs(wl) * alb

    return W


def func_sun_DOP(allvars, L):
    ...
    for i in range(len(crds1)):
        ...
        x_sol = np.sin(cSOL1[i]) * np.cos(cSOL2[i])
        y_sol = np.sin(cSOL1[i]) * np.sin(cSOL2[i])
        z_sol = np.cos(cSOL1[i])
        ...
        # Sol
        Sepsip = np.zeros(2, dtype=float)
        Sepsil = np.zeros(2, dtype=float)

        # epsilon+ definição
        Sepsip[0] = (X_sol + L * np.cos(cSOL2[i])) / (1 - L * np.cos(cSOL2[i]) * X_sol)
        Sepsip[1] = (Y_sol + L * np.sin(cSOL2[i])) / (1 - L * np.sin(cSOL2[i]) * Y_sol)
        SEp = Sepsip[0] + 1j * Sepsip[1]
        # -1/ (conjugado do epsilon+)
        SEpp = -1 / np.conj(SEp)

        # epsilon- definição
        Sepsil[0] = (X_sol - L * np.cos(cSOL2[i])) / (1 + L * np.cos(cSOL1[i]) * X_sol)
        Sepsil[1] = (Y_sol - L * np.sin(cSOL2[i])) / (1 + L * np.sin(cSOL2[i]) * Y_sol)
        SEl = Sepsil[0] + 1j * Sepsil[1]
        # -1/ (conjugado do epsilon+)
        SEll = -1 / np.conj(SEl)
        ...
        wl = ((xyo - LEp) * (xyo - LEl) * (xyo - LEpp) * (xyo - LEll)) / (
                ((1 + (np.abs(xyo) ** 2)) ** 2) * np.abs(LEp - LEpp) * np.abs(LEl - LEll)) * par_wave[i]

        ws = ((xyo - SEp) * (xyo - SEl) * (xyo - SEpp) * (xyo - SEll)) / (
                ((1 + (np.abs(xyo) ** 2)) ** 2) * np.abs(SEp - SEpp) * np.abs(LEl - SEll)) * par_wave[i]

        alt = 90 - 180 / np.pi * cSOL1[i]

        if alt < -18:
            DOP[i] = np.abs(wl) * alb[i]
        if alt >= -18:
            DOP[i] = np.abs(ws) + np.abs(wl) * alb[i]
...
def func_wav_DOP(allvars, L, c):
    ...
    for i in range(len(crds1)):
        ...
        wl = ((xyo - LEp) * (xyo - LEl) * (xyo - LEpp) * (xyo - LEll)) / (
                ((1 + (np.abs(xyo) ** 2)) ** 2) * np.abs(LEp - LEpp) * np.abs(LEl - LEll)) * banda[i] ** c

        DOP[i] = np.abs(wl)  # * (np.pi/2 - cLUA1[i])

    return DOP


def func_simple_wav_DOP(theta_field, phi_field, theta_lua, phi_lua, banda, L, c):
    x = np.sin(theta_field) * np.cos(phi_field)
    y = np.sin(theta_field) * np.sin(phi_field)
    z = np.cos(theta_field)
    ...
    wl = ((xyo - LEp) * (xyo - LEl) * (xyo - LEpp) * (xyo - LEll)) / (
            ((1 + (np.abs(xyo) ** 2)) ** 2) * np.abs(LEp - LEpp) * np.abs(LEl - LEll)) * banda ** c

    DOP = np.abs(wl)  # * (np.pi/2 - theta_lua)

    return DOP


def func_simple_wav(theta_field, phi_field, theta_lua, phi_lua, banda, seeing, n_par, k_par, d_par, L, c):
    G = func_gamma(theta_field, phi_field, theta_lua, phi_lua)
    ...
    DOP = np.abs(wl) * (np.sin(G) * banda ** c) * (np.cos(theta_lua) ** (1 / n_par)) * np.exp(
        -seeing / k_par + d_par)  # * (np.pi/2 - theta_lua)

    return DOP


def func_wav(allvars, L, c):
    crds1, crds2, cLUA1, cLUA2, banda, seeing, N, k, d = allvars
    DOP = np.zeros(len(cLUA1), dtype=float)
    DOP[:] = np.nan
    ...
    for i in range(len(crds1)):
        G = func_gamma(crds1[i], crds2[i], cLUA1[i], cLUA2[i])
        ...
        DOP[i] = np.abs(wl) * np.cos(cLUA1[i]) ** (1 / N[i]) * np.exp(-seeing[i] / k[i] + d[i]) * (np.sin(G) * banda[i] ** c)  # * (np.pi/2 - cLUA1[i])
...
def func_DOP(allvars, L, N, k, p):
    crds1, crds2, cLUA1, cLUA2, cSOL1, cSOL2, alb, seeing, wave_par = allvars
    DOP = np.zeros(len(cLUA1), dtype=float)
    DOP[:] = np.nan
    ...
    for i in range(len(crds1)):
        ...
        ws = ((xyo - SEp) * (xyo - SEl) * (xyo - SEpp) * (xyo - SEll)) / (
                ((1 + (np.abs(xyo) ** 2)) ** 2) * np.abs(SEp - SEpp) * np.abs(LEl - SEll))

        alt = 90 - 180 / np.pi * cSOL1[i]

        if alt < -18:
            DOP[i] = np.abs(wl) * alb[i] * np.cos(cLUA1[i]) ** (1 / N) * np.exp(-seeing[i] / k + p) * wave_par[
                i]  # * (np.pi - cLUA1[i])
        if alt >= -18:
            DOP[i] = (np.abs(ws) * np.cos(cSOL1[i]) ** (1 / N) + np.abs(wl) * alb[i] * np.cos(
                cLUA1[i]) ** (1 / N)) * np.exp(-seeing[i] / k + p) * wave_par[i]

    return DOP


def func_simple_DOP(theta_field, phi_field, theta_lua, phi_lua, theta_sol, phi_sol, alb, seeing, wave_par, L, N, k, p):
    global W
    ...
    if (90 - 180 / np.pi * theta_sol < -18).any():
        W = np.abs(wl) * alb * np.cos(theta_lua) ** (1 / N) * np.exp(
            -seeing / k + p) * wave_par  # * (np.pi-theta_lua)
    if (90 - 180 / np.pi * theta_sol >= -18).any():
        W = (np.abs(ws) * np.cos(theta_sol) ** (1 / N) + np.abs(wl) * alb * np.cos(
            theta_lua) ** (1 / N)) * np.exp(-seeing / k + p) * wave_par
    ...
    for t in MED:
        ...
        if P1 == 'complex':
            ...
            if P2 == 'fit wavelength':
                ...
                b = np.exp(-seen[k] / k1 + k2) * np.cos(t_lua) ** (1 / N)
                a = wave ** n
        ...
        for n in range(0, len(RA)):
            ...
            x = np.sin(tfield) * np.cos(phifield)
            y = np.sin(tfield) * np.sin(phifield)
            z = np.cos(tfield)
        ...
        for Eo in np.linspace(0, np.pi / 2, 100, endpoint=True):
            for Azo in np.linspace(0, 2 * np.pi, 400, endpoint=True):
                ...
                x_sky = np.sin(to) * np.cos(phio)
                y_sky = np.sin(to) * np.sin(phio)
                z_sky = np.cos(to)

                X_sky = x_sky / (1 - z_sky)
                Y_sky = y_sky / (1 - z_sky)

                xy_sky = X_sky + 1j * Y_sky

                w = ((xy_sky - LEp) * (xy_sky - LEl) * (xy_sky - LEpp) * (xy_sky - LEll)) / (
                        ((1 + (np.abs(xy_sky) ** 2)) ** 2) * np.abs(LEp - LEpp) * np.abs(LEl - LEll))

                dop[i, j] = np.abs(w) * a * b
        ...
        for y in range(0, len(RA)):
            if 0 <= C1field[y] <= np.pi / 2:
                if t == MED[y]:
                    ...
                    ax.annotate('observed field ' + field[y] + ': banda ' + str(BANDA[y]) + '\n DoP: ' + str(
                        round(POL_OBS[y], 3)) + ' $ \pm $ ' + str(round(errPOL_OBS[y], 3)) + '\n DoP sim: ' + str(
                        round(W_field[y], 3)), xy=(C2field[y], C1field[y] * 180 / np.pi), xycoords='data',
        ...
        plt.colorbar()
        nome = 'MAP_multi_' + P1 + '_' + P2 + '_' + str(d) + '.png'
        if band is not None:
            nome = 'MAP_multi_' + P1 + '_' + P2 + '_' + str(band) + '_' + str(d) + '.png'
        plt.savefig(nome)
...
def FIT(band=None, condition=None, command='ALL'):
    global conjunto, cor_line, cor, lab_wave, cor_unc, par_wave, Rpar, barra, B_par, B_par_mix, V_par_mix, R_par, R_par_mix, I_par, I_par_mix, V_par, result_data
    df = pd.read_csv('data.csv', sep=';')
    ...
    if command == 'ALL' or command == 'fit wavelength' or command == 'individuals mix' or command == 'horizon fit' or command == 'fit seeing':
        B_par_mix, B_chisqr_mix, B_bic_mix, result_data_B_mix = field_functions.fit_base('B', condition,
                                                                                         command='simple_mix_multi')
        V_par_mix, V_chisqr_mix, V_bic_mix, result_data_V_mix = field_functions.fit_base('V', condition,
                                                                                         command='simple_mix_multi')
        R_par_mix, R_chisqr_mix, R_bic_mix, result_data_R_mix = field_functions.fit_base('R', condition,
                                                                                         command='simple_mix_multi')
        I_par_mix, I_chisqr_mix, I_bic_mix, result_data_I_mix = field_functions.fit_base('I', condition,
                                                                                         command='simple_mix_multi')

        L_ind_mix = [B_par_mix[0], V_par_mix[0], R_par_mix[0], I_par_mix[0]]
        N_ind_mix = [B_par_mix[1], V_par_mix[1], R_par_mix[1], I_par_mix[1]]
        k_ind_mix = [B_par_mix[2], V_par_mix[2], R_par_mix[2], I_par_mix[2]]
        d_ind_mix = [B_par_mix[3], V_par_mix[3], R_par_mix[3], I_par_mix[3]]
        ...
        df_ind_mix = pd.DataFrame(
            {'L': L_ind_mix, 'N': N_ind_mix, '$k_{1}$': k_ind_mix, '$k_{2}$': d_ind_mix, 'Chi-square': chisqr_ind_mix,
             'BIC': bic_ind_mix},
            index=index)
        columns = ['L', 'N', 'k', 'd', 'Chi-square', 'BIC']

        df_ind_mix.plot(rot=0, subplots=True, style=".--")
        plt.savefig('indvidual_multi_dop_mix_fit_separate_parameters.png')
        plt.pause(2)
        plt.close()
        ...
        field_functions.plot_all(result_data_mix, command='simple_mix_multi')

        if command == 'individuals mix':
            result_data_mix.to_csv("output.csv")
            exit(df_ind_mix)

    if command == 'ALL' or command == 'fit wavelength' or command == 'individuals regular' or command == 'horizon fit' or command == 'fit seeing' or command == 'fit Sun' or command == 'fit all':
        B_par, B_chisqr, B_bic, result_data_B = field_functions.fit_base('B', condition,
                                                                         command='simple_regular_multi')
        ...
        L_ind_reg = [B_par[0], V_par[0], R_par[0], I_par[0]]
        par_ind_reg = [B_par[1], V_par[1], R_par[1], I_par[1]]
        chisqr_ind_reg = [B_chisqr, V_chisqr, R_chisqr, I_chisqr]
        bic_ind_reg = [B_bic, V_bic, R_bic, I_bic]
        index = ['B', 'V', 'R', 'I']

        df_ind_reg = pd.DataFrame({'L': L_ind_reg, '$A$': par_ind_reg, 'Chi-square': chisqr_ind_reg, 'BIC': bic_ind_reg}, index=index)
    ...
    if command == 'ALL' or command == 'individuals wave':
        B_par, B_chisqr, B_bic, result_data_B = field_functions.fit_base('B', condition, command='simple_wave_multi')
        V_par, V_chisqr, V_bic, result_data_V = field_functions.fit_base('V', condition, command='simple_wave_multi')
        R_par, R_chisqr, R_bic, result_data_R = field_functions.fit_base('R', condition, command='simple_wave_multi')
        I_par, I_chisqr, I_bic, result_data_I = field_functions.fit_base('I', condition, command='simple_wave_multi')

        # guarda e junta os dados de todos os fits num data frame para todas as bandas
        ...
        # field_functions.plot_all_bands(B_par, V_par, R_par, I_par, condition, command='regular_multi')
        field_functions.plot_all(result_data, command='simple_wave_multi')

        L_ind = [B_par[0], V_par[0], R_par[0], I_par[0]]
        c_ind = [B_par[1], V_par[1], R_par[1], I_par[1]]
        chisqr_ind = [B_chisqr, V_chisqr, R_chisqr, I_chisqr]
        bic_ind = [B_bic, V_bic, R_bic, I_bic]
        index = ['B', 'V', 'R', 'I']

        df_ind = pd.DataFrame({'L': L_ind, 'c': c_ind, 'Chi-square': chisqr_ind, 'BIC': bic_ind}, index=index)

        df_ind.plot(rot=0, subplots=True, style=".--")
        plt.savefig('indvidual_multi_wave_fit_separate_parameters.png')
        plt.pause(2)
        plt.close()
    ...
    if command == 'ALL' or command == 'individuals seeing':
        B_par_seen, B_chisqr_seen, B_bic, result_data_B_seen = field_functions.fit_base('B', condition,
                                                                                     command='simple_seeing_multi')
        V_par_seen, V_chisqr_seen, V_bic, result_data_V_seen = field_functions.fit_base('V', condition,
                                                                                     command='simple_seeing_multi')
        R_par_seen, R_chisqr_seen, R_bic, result_data_R_seen = field_functions.fit_base('R', condition,
                                                                                     command='simple_seeing_multi')
        I_par_seen, I_chisqr_seen, I_bic, result_data_I_seen = field_functions.fit_base('I', condition,
                                                                                     command='simple_seeing_multi')

        L_ind_seen = [B_par_seen[0], V_par_seen[0], R_par_seen[0], I_par_seen[0]]
        k_ind_seen = [B_par_seen[1], V_par_seen[1], R_par_seen[1], I_par_seen[1]]
        d_ind_seen = [B_par_seen[2], V_par_seen[2], R_par_seen[2], I_par_seen[2]]
        chisqr_ind_seen = [B_chisqr_seen, V_chisqr_seen, R_chisqr_seen, I_chisqr_seen]
        bic_ind_seen = [B_bic, V_bic, R_bic, I_bic]
        index = ['B', 'V', 'R', 'I']

        df_ind_seen = pd.DataFrame({'L': L_ind_seen, '$k_{1}$': k_ind_seen, '$k_{2}$': d_ind_seen, 'Chi-square': chisqr_ind_seen, 'BIC': bic_ind_seen},
                                  index=index)

        df_ind_seen.plot(rot=0, subplots=True, style=".--")
        plt.savefig('indvidual_multi_dop_seen_fit_separate_parameters.png')
        plt.pause(2)
        plt.close()
        ...
        # field_functions.plot_all_bands(B_par, V_par, R_par, I_par, condition, command='regular_multi')
        field_functions.plot_all(result_data_seen, command='simple_seen_multi')
    ...
    for n in range(0, total):
        ...
        Q_OBS.append(float(Qval[n]))
        errQ_OBS.append(float(errQval[n]))
        U_OBS.append(float(Uval[n]))
        errU_OBS.append(float(errUval[n]))
    ...
    WAV, N_BAND = np.asarray(WAV, dtype=np.float32), np.asarray(N_BAND, dtype=np.float32)
    K_BAND, D_BAND = np.asarray(K_BAND, dtype=np.float32), np.asarray(D_BAND, dtype=np.float32)
    ...
    if command == 'ALL' or command == 'individuals horizon':
        B_par_hor, B_chisqr_hor, B_bic, result_data_B_hor = field_functions.fit_base('B', condition,
                                                                              command='simple_hor_multi')
        V_par_hor, V_chisqr_hor, V_bic, result_data_V_hor = field_functions.fit_base('V', condition,
                                                                              command='simple_hor_multi')
        R_par_hor, R_chisqr_hor, R_bic, result_data_R_hor = field_functions.fit_base('R', condition,
                                                                              command='simple_hor_multi')
        I_par_hor, I_chisqr_hor, I_bic, result_data_I_hor = field_functions.fit_base('I', condition,
                                                                              command='simple_hor_multi')

        L_ind_hor = [B_par_hor[0], V_par_hor[0], R_par_hor[0], I_par_hor[0]]
        par_ind_hor = [B_par_hor[1], V_par_hor[1], R_par_hor[1], I_par_hor[1]]
        chisqr_ind_hor = [B_chisqr_hor, V_chisqr_hor, R_chisqr_hor, I_chisqr_hor]
        bic_ind_hor = [B_bic, V_bic, R_bic, I_bic]
        index = ['B', 'V', 'R', 'I']

        df_ind_hor = pd.DataFrame({'L': L_ind_hor, 'N': par_ind_hor, 'Chi-square': chisqr_ind_hor, 'BIC': bic_ind_hor}, index=index)

        df_ind_hor.plot(rot=0, subplots=True, style=".--")
        plt.savefig('indvidual_multi_dop_hor_fit_separate_parameters.png')
        plt.pause(2)
        plt.close()
        ...
        # field_functions.plot_all_bands(B_par, V_par, R_par, I_par, condition, command='regular_multi')
        field_functions.plot_all(result_data_hor, command='simple_hor_multi')
    ...
    if command == 'ALL' or command == 'horizon fit':
        ...
        model.set_param_hint('N', min=0, max=50)
        ...
        result_emcee_hor = [result.params['L'].value, result.params['N'].value]
        result_emcee_hor = np.asarray(result_emcee_hor)
        Rpar = result_emcee_hor

        # data_parameters.append({'L': result.params['L'].value, 'N': result.params['N'].value, 'chi-square': result.chisqr}, ignore_index=True)

        txname = 'REPORT_' + LABEL + '_' + LAB + '_horizon_multi_' + meto + '.txt'
        model_name = 'MODEL_' + LABEL + '_' + LAB + '_horizon_multi_' + meto + '.sav'
        ...
        if band is None or band is BAN:
            ...
            k1 = np.add(b1, c1)
            k2 = np.subtract(b1, c1)
            plt.fill_between(a1, k2, k1, where=(k2 < k1), interpolate=True, color='lavender')

            l1 = np.add(b2, c2)
            l2 = np.subtract(b2, c2)
            plt.fill_between(a2, l2, l1, where=(l2 < l1), interpolate=True, color='beige')

            m1 = np.add(b3, c3)
            m2 = np.subtract(b3, c3)
            plt.fill_between(a3, m2, l1, where=(m2 < m1), interpolate=True, color='mistyrose')

            n1 = np.add(b4, c4)
            n2 = np.subtract(b4, c4)
            plt.fill_between(a4, n2, n1, where=(n2 < n1), interpolate=True, color='antiquewhite')

            plt.ylim(0, 0.8)
            if isinstance(result.params['L'].stderr, float):
                L_par = round(result.params['L'].stderr, 3)
            else:
            ...
            label_text = 'fit parameters: L = ' + str(round(result.params['L'].value, 3)) + '$\pm$' + str(
                L_par) + ',   $N$ = ' + str(round(result.params['N'].value, 3)) + '$\pm$' + str(
                N_par) + '\n' + 'chi-square: ' + str(
                round(result.chisqr, 7)) + ',   reduced chi-square: ' + str(
                round(result.redchi, 7)) + '\n' + 'Bayesian Information Criterion: ' + str(round(result.bic, 2))
            ...
            leg_text = 'Individuals amplituded\n \n$A_{B}$= ' + str(
                round(B_par[1], 3)) + '  \n$A_{V}$= ' + str(round(V_par[1], 3)) + '  \n$A_{R}$= ' + str(
                round(R_par[1], 3)) + '  \n$A_{I}$= ' + str(round(I_par[1], 3))
            ...
            plt.savefig('IMAGE_' + LABEL + '_' + LAB + '_horizon_multi_' + meto + '.png', bbox_inches='tight')
        ...
        model = lmfit.Model(func_simple_hor_DOP,
                            independent_vars=['theta_field', 'phi_field', 'theta_lua', 'phi_lua', 'par_wave'])
        model.set_param_hint('L', min=0, max=1)
        model.set_param_hint('N', min=0, max=50)
        p = model.make_params(L=np.random.rand(), N=np.random.rand())
        result_simple = model.fit(data=POL_OBS, params=p, theta_field=C1field, phi_field=C2field, theta_lua=C1lua,
                                  phi_lua=C2lua, par_wave=WAV, weights=errPOL_OBS, method=meto)

        result_simple_hor = [result_simple.params['L'].value, result_simple.params['N'].value]
        result_simple_hor = np.asarray(result_simple_hor)

        # data_parameters.append({'L': result_simple.params['L'].value, 'N': result_simple.params['N'].value, 'chi-square': result_simple.chisqr}, ignore_index=True)
        ...
        model_name = 'MODEL_' + LABEL + '_' + LAB + '_simple_horizon_multi_' + meto + '.sav'

        lmfit.model.save_modelresult(result_simple, model_name)

        y1 = func_simple_hor_DOP(theta_field=C1field, phi_field=C2field, theta_lua=C1lua, phi_lua=C2lua,
                                 par_wave=WAV,
                                 L=result_simple_hor[0], N=result_simple_hor[1])
        ...
        if band is None or band is BAN:
            ...
            label_text = 'fit parameters: L = ' + str(round(result_simple.params['L'].value, 3)) + '$\pm$' + str(
                ...
                N_par) + '\n' + 'chi-square: ' + str(round(result_simple.chisqr, 7)) + ',   reduced chi-square: ' + str(
                round(result_simple.redchi, 7)) + '\n' + 'Bayesian Information Criterion: ' + str(
            ...
            plt.ylabel('Residual data')
            plt.grid(True)
            plt.savefig('IMAGE_' + LABEL + '_' + LAB + '_simple_horizon_multi_' + meto + '.png',
                        bbox_inches='tight')
            ...
            for item in band:
                ...
                plt.annotate(label_text, xy=(0.1, 0.2), xycoords='axes fraction', xytext=(0.1, 1),
                             ...
                             bbox=dict(boxstyle="round", fc="w"))
                leg_text = 'fit adding \nthe horizon correction parameter...\n \n$c_{B}$= ' + str(
                    round(B_par[1], 3)) + '  \n$c_{V}$= ' + str(round(V_par[1], 3)) + '  \n$c_{R}$= ' + str(
                    round(R_par[1], 3)) + '  \n$c_{I}$= ' + str(round(I_par[1], 3))
                plt.annotate(leg_text, xy=(0.1, 0.2), xycoords='axes fraction', xytext=(1.05, 0.1),
                             textcoords='axes fraction',
                             horizontalalignment='left', verticalalignment='center',
                             bbox=dict(boxstyle="round", fc="w"))
    ...
    if command == 'ALL' or command == 'fit Sun':
        ...
        p = model.make_params(L=np.random.rand())
        ...
        result = model.fit(data=POL_OBS, params=p,
                           ...
                           method=meto, weights=errPOL_OBS)

        result_emcee_sun = [result.params['L'].value]
        result_emcee_sun = np.asarray(result_emcee_sun)
        Rpar = result_emcee_sun

        # data_parameters.append({'L': result.params['L'].value, 'chi-square': result.chisqr}, ignore_index=True)

        txname = 'REPORT_' + LABEL + '_' + LAB + '_sun_multi_' + meto + '.txt'
        TXT = open(txname, "w+")
        ...
        model_name = 'MODEL_' + LABEL + '_' + LAB + '_sun_multi_' + meto + '.sav'
        ...
        if band is None or band is BAN:
            ...
            plt.ylabel('Polarization')
            label_text = 'fit parameters: L = ' + str(round(result.params['L'].value, 3)) + '$\pm$' + str(
                round(result.params['L'].stderr, 3)) + '\n' + 'chi-square: ' + str(
                round(result.chisqr, 10)) + ',   reduced chi-square: ' + str(
                round(result.redchi, 10)) + '\n' + 'Bayesian Information Criterion: ' + str(round(result.bic, 2))
            plt.annotate(label_text, xy=(0.1, 0.2), xycoords='axes fraction', xytext=(0.1, 1),
                         textcoords='axes fraction',
            ...
            plt.ylabel('Residual data')
            plt.grid(True)
            plt.savefig('IMAGE_' + LABEL + '_' + LAB + '_sun_multi_' + meto + '.png', bbox_inches='tight')
        ...
        model = lmfit.Model(func_simple_sun_DOP,
                            independent_vars=['theta_field', 'phi_field', 'theta_lua', 'phi_lua', 'theta_sol',
                                              'phi_sol',
                                              'alb', 'par_wave'])
        model.set_param_hint('L', min=0, max=0.1)
        p = model.make_params(L=np.random.rand())
        result_simple = model.fit(data=POL_OBS, params=p, theta_field=C1field, phi_field=C2field, theta_lua=C1lua,
                                  phi_lua=C2lua, theta_sol=C1sol, phi_sol=C2sol, alb=ALBEDO, par_wave=WAV,
                                  weights=errPOL_OBS,
                                  method='meto')

        result_simple_sun = [result_simple.params['L'].value]  # , result.params['N'].value]
        result_simple_sun = np.asarray(result_simple_sun)
        # data_parameters.append({'L': result_simple.params['L'].value, 'chi-square': result_simple.chisqr}, ignore_index=True)
        ...
        model_name = 'MODEL_' + LABEL + '_' + LAB + '_simple_sun_multi_' + meto + '.sav'

        lmfit.model.save_modelresult(result_simple, model_name)

        y1 = func_simple_sun_DOP(theta_field=C1field, phi_field=C2field, theta_lua=C1lua, phi_lua=C2lua,
                                 theta_sol=C1sol,
                                 phi_sol=C2sol, alb=ALBEDO, par_wave=WAV, L=result_simple_sun[0])
        ...
        if band is None or band is BAN:
            ...
            if isinstance(result.params['L'].stderr, float):
                ...
                L_par = result.params['L'].stderr
            label_text = 'fit parameters: L = ' + str(round(result_simple.params['L'].value, 3)) + '$\pm$' + str(
                L_par) + '\n' + 'chi-square: ' + str(round(result_simple.chisqr, 10)) + ',  reduced chi-square: ' + str(
            ...
            plt.ylabel('Residual data')
            plt.grid(True)
            plt.savefig('IMAGE_' + LABEL + '_' + LAB + '_simple_sun_multi_' + meto + '.png', bbox_inches='tight')
    ...
    if command == 'ALL' or command == 'fit seeing':
        ...
        model.set_param_hint('p', min=0.1, max=1)
        p = model.make_params(L=np.random.rand(), k=np.random.rand(), p=np.random.rand())  # , N=10)
        ...
        result_emcee_seeing = [result.params['L'].value, result.params['k'].value, result.params['p'].value]
        result_emcee_seeing = np.asarray(result_emcee_seeing)
        Rpar = result_emcee_seeing

        # data_parameters.append({'L': result.params['L'].value, '$k_{1}$': result.params['k'].value, '$k_{2}$': result.params['p'].value,'chi-square': result.chisqr}, ignore_index=True)

        txname = 'REPORT_' + LABEL + '_' + LAB + '_seeing_multi_' + meto + '.txt'
        TXT = open(txname, "w+")

        model_fit_report = result.fit_report()
        TXT.write('***  Fit: seeing  astronomical parameter  *** \n \n')
        TXT.write('Independent variables: \n')
        TXT.write(str(model.independent_vars))
        TXT.write(model_fit_report)
        TXT.write('\n \n')
        ...
        if band is None or band is BAN:
            ...
            plt.ylabel('Polarization')
            label_text = 'fit parameters: L = ' + str(round(result.params['L'].value, 3)) + '$\pm$' + str(
                round(result.params['L'].stderr, 3)) + ',   $k_{1}$ = ' + str(
            ...
            plt.ylabel('Residual data')
            plt.grid(True)
            plt.savefig('IMAGE_' + LABEL + '_' + LAB + '_seeing_multi_' + meto + '.png', bbox_inches='tight')
        ...
        model = lmfit.Model(func_simple_seeing_DOP,
                            independent_vars=['theta_field', 'phi_field', 'theta_lua', 'phi_lua', 'seeing',
                                              'par_wave'])
        model.set_param_hint('L', min=-0.1, max=0.1)
        model.set_param_hint('k', min=3, max=20)
        model.set_param_hint('p', min=0.1, max=1)
        p = model.make_params(L=np.random.rand(), k=np.random.rand(), p=np.random.rand())
        result_simple = model.fit(data=POL_OBS, params=p, theta_field=C1field, phi_field=C2field, theta_lua=C1lua,
                                  phi_lua=C2lua, seeing=SEEING, par_wave=WAV, weights=errPOL_OBS,
                                  method=meto)

        result_simple_seeing = [result_simple.params['L'].value, result_simple.params['k'].value,
                                result_simple.params['p'].value]
        result_simple_seeing = np.asarray(result_simple_seeing)

        # data_parameters.append({'L': result_simple.params['L'].value, '$k_{1}$': result_simple.params['k'].value, '$k_{2}$': result_simple.params['p'].value, 'chi-square': result_simple.chisqr}, ignore_index=True)

        model_fit_report = result_simple.fit_report()
        TXT.write('***  Eighth Fit: now considering the seeing  astronomical parameter in a simplier way *** \n \n')
        TXT.write('Independent variables: \n')
        TXT.write(str(model.independent_vars))
        TXT.write(model_fit_report)
        TXT.write('\n \n')
        ...
        y1 = func_simple_seeing_DOP(theta_field=C1field, phi_field=C2field, theta_lua=C1lua, phi_lua=C2lua,
                                    seeing=SEEING,
                                    par_wave=WAV,
                                    L=result_simple_seeing[0], k=result_simple_seeing[1], p=result_simple_seeing[2])
        ...
        if band is None or band is BAN:
            ...
            if isinstance(result.params['L'].stderr, float):
                ...
                L_par = result.params['L'].stderr
            label_text = 'fit parameters: L = ' + str(round(result_simple.params['L'].value, 3)) + '$\pm$' + str(
                L_par) + ',   $k_{1}$ = ' + str(round(result_simple.params['k'].value, 3)) + '$\pm$' + str(
            ...
            plt.ylabel('Residual data')
            plt.grid(True)
            plt.savefig('IMAGE_' + LABEL + '_' + LAB + '_simple_seeing_multi_' + meto + '.png',
                        bbox_inches='tight')
    ...
    if command == 'ALL' or command == 'fit wavelength':
        ...
        p = model.make_params(L=np.random.rand(), c=np.random.rand())  # , N=10)
        # result = model.fit(data=POL_OBS, params=p, allvars=[C1field, C2field, C1lua, C2lua, BAND], method='emcee',
        #                    weights=errPOL_OBS, nfev=500000)
        result = model.fit(data=POL_OBS, params=p,
                           allvars=[C1field, C2field, C1lua, C2lua, BAND, SEEING, N_BAND, K_BAND, D_BAND],
                           weights=errPOL_OBS, method=meto)

        result_emcee_wav = [result.params['L'].value, result.params['c'].value]
        result_emcee_wav = np.asarray(result_emcee_wav)
        Rpar = result_emcee_wav

        # data_parameters.append({'L': result.params['L'].value, 'c': result.params['c'].value, 'chi-square': result.chisqr},ignore_index=True)

        txname = 'REPORT_' + LABEL + '_' + LAB + '_wave_multi_' + meto + '.txt'
        TXT = open(txname, "w+")

        model_fit_report = result.fit_report()
        TXT.write('***  Fit: now considering the wavelength of light *** \n \n')
        TXT.write('Independent variables: \n')
        TXT.write(str(model.independent_vars))
        TXT.write(model_fit_report)
        TXT.write('\n \n')

        model_name = 'MODEL_' + LABEL + '_' + LAB + '_wave_multi_' + meto + '.sav'
        ...
        fit_observations_resume.to_csv("output.csv")

        if band is None or band is BAN:
            fig_x = plt.figure(figsize=(12, 5))
            ox = fig_x.add_axes((.1, .3, .5, .6))
            ...
            for d in range(0, len(a1)):
                label1.append('Field observed in band B: \n $\u03B8_{FIELD}$ = ' + str(
                    round(i1[d] * 180 / np.pi, 2)) + ' ; $\u03C6_{FIELD}$ = ' + str(
                    round(j1[d] * 180 / np.pi, 2)) + ';\n$\u03B8_{MOON}$ = ' + str(
                    round(g1[d] * 180 / np.pi, 2)) + ' ; $\u03C6_{MOON}$ = ' + str(
                    round(h1[d] * 180 / np.pi, 2)) + ';\n$DoP_{OBS}$ = ' + str(
                    round(e1[d], 3)) + ' $ \pm $ ' + str(round(f1[d], 3)) + ';\n$DoP_{SIM}$ = ' + str(
                    round(b1[d], 3)) + ' $ \pm $ ' + str(round(c1[d], 3)))
            ...
            for d in range(0, len(a2)):
                label2.append('Field observed in band V: \n $\u03B8_{FIELD}$ = ' + str(
                    round(i2[d] * 180 / np.pi, 2)) + ' ; $\u03C6_{FIELD}$ = ' + str(
                    round(j2[d] * 180 / np.pi, 2)) + ';\n$\u03B8_{MOON}$ = ' + str(
                    round(g2[d] * 180 / np.pi, 2)) + ' ; $\u03C6_{MOON}$ = ' + str(
                    round(h2[d] * 180 / np.pi, 2)) + ';\n$DoP_{OBS}$ = ' + str(round(e2[d], 3)) + ' $ \pm $ ' + str(
                    round(f2[d], 3)) + ';\n$DoP_{SIM}$ = ' + str(round(b2[d], 3)) + ' $ \pm $ ' + str(
                    round(c2[d], 3)))
            ...
            for d in range(0, len(a3)):
                label3.append('Field observed in band R: \n $\u03B8_{FIELD}$ = ' + str(
                    round(i3[d] * 180 / np.pi, 2)) + ' ; $\u03C6_{FIELD}$ = ' + str(
                    round(j3[d] * 180 / np.pi, 2)) + ';\n$\u03B8_{MOON}$ = ' + str(
                    round(g3[d] * 180 / np.pi, 2)) + ' ; $\u03C6_{MOON}$ = ' + str(
                    round(h3[d] * 180 / np.pi, 2)) + ';\n$DoP_{OBS}$ = ' + str(round(e3[d], 3)) + ' $ \pm $ ' + str(
                    round(f3[d], 3)) + ';\n$DoP_{SIM}$ = ' + str(round(b3[d], 3)) + ' $ \pm $ ' + str(
                    round(c3[d], 3)))
            ...
            for d in range(0, len(a4)):
                label4.append('Field observed in band I: \n $\u03B8_{FIELD}$ = ' + str(
                    round(i4[d] * 180 / np.pi, 2)) + ' ; $\u03C6_{FIELD}$ = ' + str(
                    round(j4[d] * 180 / np.pi, 2)) + ';\n$\u03B8_{MOON}$ = ' + str(
                    round(g4[d] * 180 / np.pi, 2)) + ' ; $\u03C6_{MOON}$ = ' + str(
                    round(h4[d] * 180 / np.pi, 2)) + ';\n$DoP_{OBS}$ = ' + str(round(e4[d], 3)) + ' $ \pm $ ' + str(
                    round(f4[d], 3)) + ';\n$DoP_{SIM}$ = ' + str(round(b4[d], 3)) + ' $ \pm $ ' + str(
                    round(c4[d], 3)))
            ...
            for x in range(0, len(a1)):
                point1, = plt.plot(a1[x], e1[x], color='blue')
                points1.append(point1)
            mplcursors.cursor(band1, highlight=True).connect("add",
                                                             lambda sel: sel.annotation.set_text(label1[sel.index]))

            plt.plot(a2, b2, '-', color='mediumseagreen', markersize=2, label='fit results V band')
            plt.errorbar(a2, e2, yerr=f2, ms=2.0, fmt='o', color='green', label='data V band')
            band2 = plt.scatter(a2, e2, color='none')
            mplcursors.cursor(band2).connect("add", lambda sel: sel.annotation.set_text(label2[sel.index]))

            plt.plot(a3, b3, '-', color='indianred', markersize=2, label='fit results R band')
            plt.errorbar(a3, e3, yerr=f3, ms=2.0, fmt='o', color='red', label='data R band')
            band3 = plt.scatter(a3, e3, color='none')
            mplcursors.cursor(band3).connect("add", lambda sel: sel.annotation.set_text(label3[sel.index]))

            plt.plot(a4, b4, '-', color='orange', markersize=2, label='fit results I band')
            plt.errorbar(a4, e4, yerr=f4, ms=2.0, fmt='o', color='darkorange', label='data I band')
            band4 = plt.scatter(a4, e4, color='none')
            mplcursors.cursor(band4).connect("add", lambda sel: sel.annotation.set_text(label4[sel.index]))
            ...
            plt.ylabel('Polarization')
            label_text = 'fit parameters: L = ' + str(round(result.params['L'].value, 3)) + '$\pm$' + str(
                round(result.params['L'].stderr, 3)) + ',   $c$ = ' + str(
                round(result.params['c'].value, 3)) + '$\pm$' + str(
                round(result.params['c'].stderr, 3)) + '\n' + 'chi-square: ' + str(
                round(result.chisqr, 10)) + ',  reduced chi-square: ' + str(
                round(result.redchi, 10)) + '\n' + 'Bayesian Information Criterion: ' + str(round(result.bic, 2))
            plt.annotate(label_text, xy=(0.1, 0.2), xycoords='axes fraction', xytext=(0.1, 1.05),
            ...
            leg_text = '     fit individuals parameters \n$N_{B}$= ' + str(
                round(B_par_mix[1], 3)) + '   $k_{B}$= ' + str(round(B_par_mix[2], 3)) + '   $d_{B}$= ' + str(
                round(B_par_mix[3], 3)) + '\n$N_{V}$= ' + str(round(V_par_mix[1], 3)) + '$  k_{V}$= ' + str(
                round(V_par_mix[2], 3)) + '$   d_{V}$= ' + str(round(V_par_mix[3], 3)) + '\n$N_{R}$= ' + str(
                round(R_par_mix[1], 3)) + '$   k_{R}$= ' + str(round(R_par_mix[2], 3)) + '$   d_{R}$= ' + str(
                round(R_par_mix[3], 3)) + '\n$N_{I}$= ' + str(round(I_par_mix[1], 3)) + '$   k_{I}$= ' + str(
                round(I_par_mix[2], 3)) + '$   d_{I}$= ' + str(round(I_par_mix[3], 3))
            plt.annotate(leg_text, xy=(0.1, 0.01), xycoords='axes fraction', xytext=(1.02, -0.1),
                         textcoords='axes fraction', fontsize=8,
                         horizontalalignment='left', verticalalignment='center', bbox=dict(boxstyle="round", fc="w"))
            ...
            plt.ylabel('Residual data')
            plt.grid(True)
            plt.savefig('IMAGE_' + LABEL + '_' + LAB + '_wave_multi_' + meto + '.png', bbox_inches='tight')
            ...
            ax = fig_x.add_subplot(122, projection='polar')
            plt.subplots_adjust(right=0.96, left=0.32, top=0.92, bottom=0.00, wspace=0.4)
            ax = plt.gca(projection='polar')
            ax.set_theta_zero_location('N')
            ax.set_rlim(1, 91)

            plt.plot(h1, g1 * 180 / np.pi, 'o', color='black', markersize=3)

            moons1 = []
            for x in range(0, len(h1)):
                moon1, = ax.plot(h1[x], g1[x] * 180 / np.pi, 'o', color='black', markersize=3)
                moons1.append(moon1)
            cursor = mplcursors.cursor(points1 + moons1, highlight=True)
            pairs = dict(zip(points1, moons1))
            pairs.update(zip(moons1, points1))

            @cursor.connect("add")
            def on_add(sel):
                sel.extras.append(cursor.add_highlight(pairs[sel.artist]))

            plt.plot(h2, g2 * 180 / np.pi, 'o', color='black', markersize=3)
            plt.plot(h3, g3 * 180 / np.pi, 'o', color='black', markersize=3)
            plt.plot(h4, g4 * 180 / np.pi, 'o', color='black', markersize=3)

            plt.plot(j1, i1 * 180 / np.pi, 'o', color='blue', markersize=3)
            plt.plot(j2, i2 * 180 / np.pi, 'o', color='green', markersize=3)
            plt.plot(j3, i3 * 180 / np.pi, 'o', color='red', markersize=3)
            plt.plot(j4, i4 * 180 / np.pi, 'o', color='darkorange', markersize=3)
        ...
        # sns.lineplot(data=fit_observations_resume, x='GAMMA', y='WAVE POL', hue='WAVELENGTH', err_style='band')

        # ________________________________________________________________________________________

        model = lmfit.Model(func_simple_wav,
                            independent_vars=['theta_field', 'phi_field', 'theta_lua', 'phi_lua', 'banda', 'seeing',
                                              'n_par', 'k_par', 'd_par'])
        model.set_param_hint('L', min=0.0, max=0.1)
        model.set_param_hint('c')
        p = model.make_params(L=np.random.rand(), c=np.random.rand())
        result_simple = model.fit(data=POL_OBS, params=p, theta_field=C1field, phi_field=C2field, theta_lua=C1lua,
                                  phi_lua=C2lua, banda=BAND, seeing=SEEING, n_par=N_BAND, k_par=K_BAND, d_par=D_BAND,
                                  weights=errPOL_OBS, method=meto)

        result_simple_wav = [result_simple.params['L'].value, result_simple.params['c'].value]
        result_simple_wav = np.asarray(result_simple_wav)

        # data_parameters.append({'L': result_simple.params['L'].value, 'c': result_simple.params['c'].value,'chi-square': result_simple.chisqr}, ignore_index=True)

        model_fit_report = result_simple.fit_report()
        TXT.write('*** Fit: now considering the wavelength in a simplier way *** \n \n')
        TXT.write('Independent variables: \n')
        TXT.write(str(model.independent_vars))
        TXT.write(model_fit_report)
        TXT.write('\n \n')

        model_name = 'MODEL_' + LABEL + '_' + LAB + '_simple_wave_multi_' + meto + '.sav'

        lmfit.model.save_modelresult(result_simple, model_name)

        y1 = func_simple_wav(theta_field=C1field, phi_field=C2field, theta_lua=C1lua, phi_lua=C2lua, banda=BAND,
                             seeing=SEEING, n_par=N_BAND, k_par=K_BAND, d_par=D_BAND, L=result_simple_wav[0],
                             c=result_simple_wav[1])
        ...
        if band is None or band is BAN:
            ...
            label_text = 'fit parameters: L = ' + str(round(result_simple.params['L'].value, 3)) + '$\pm$' + str(
                L_par) + ',   $c$ = ' + str(round(result_simple.params['c'].value, 3)) + '$\pm$' + str(
                c_par) + '\n' + 'chi-square: ' + str(
                round(result_simple.chisqr, 10)) + ',  reduced chi-square: ' + str(
                round(result_simple.redchi, 10)) + '\n' + 'Bayesian Information Criterion: ' + str(
                round(result_simple.bic, 2))
            ...
            plt.errorbar(a2, d2, yerr=f2, ms=2.0, fmt='o', color='green', label='diff V band')
            plt.plot(a2, c2, '-', color='mediumseagreen', markersize=2, label='uncertanties fit')
            plt.errorbar(a3, d3, yerr=f3, ms=2.0, fmt='o', color='red', label='diff R band')
            plt.plot(a3, c3, '-', color='indianred', markersize=2, label='uncertanties fit')
            plt.errorbar(a4, d4, yerr=f4, ms=2.0, fmt='o', color='darkorange', label='diff V band')
            plt.plot(a4, c4, '-', color='orange', markersize=2, label='uncertanties fit')
            ...
            plt.ylabel('Residual data')
            plt.grid(True)
            plt.savefig('IMAGE_' + LABEL + '_' + LAB + '_simple_wave_multi_' + meto + '.png', bbox_inches='tight')
            barra.update(5)
            # plt.pause(2)
            # plt.close()
    ...
    if command == 'ALL' or command == 'fit all':
        ...
        model.set_param_hint('k', min=3, max=20)
        model.set_param_hint('p', min=0.1, max=1)
        model.set_param_hint('N', min=3, max=20)
        p = model.make_params(L=np.random.rand(), N=np.random.rand(), k=np.random.rand(), p=np.random.rand())  # , N=10)
        # result = model.fit(data=POL_OBS, params=p, allvars=[C1field, C2field, C1lua, C2lua, C1sol, C2sol, ALBEDO,
        # SEEING, BAND], method='emcee', weights=errPOL_OBS, nfev=500000)
        result = model.fit(data=POL_OBS, params=p,
                           allvars=[C1field, C2field, C1lua, C2lua, C1sol, C2sol, ALBEDO, SEEING, WAV],
                           weights=errPOL_OBS, method=meto)

        result_emcee = [result.params['L'].value, result.params['N'].value, result.params['k'].value,
                        result.params['p'].value]
        result_emcee = np.asarray(result_emcee)
        Rpar = result_emcee

        # data_parameters.append({'L': result.params['L'].value, '$k_{1}$': result.params['k'].value, '$k_{2}$': result.params['p'].value,'N': result.params['N'].value, 'c': result.params['c'].value, 'chi-square': result.chisqr}, ignore_index=True)

        txname = 'REPORT_' + LABEL + '_' + LAB + '_all_multi_' + meto + '.txt'
        TXT = open(txname, "w+")
        ...
        model_name = 'MODEL_' + LABEL + '_' + LAB + '_all_multi_' + meto + '.sav'

        lmfit.model.save_modelresult(result, model_name)

        y1 = func_DOP([C1field, C2field, C1lua, C2lua, C1sol, C2sol, ALBEDO, SEEING, WAV], *result_emcee)
        ...
        if band is None or band is BAN:
            ...
            plt.ylabel('Polarization')
            label_text = 'fit parameters: L = ' + str(round(result.params['L'].value, 3)) + '$\pm$' + str(
                L_par) + ',   $k_{1}$ = ' + str(round(result.params['k'].value, 3)) + '$\pm$' + str(
                k_par) + ',   $k_{2}$ = ' + str(round(result.params['p'].value, 3)) + '$\pm$' + str(
                p_par) + ',   $N$ = ' + str(round(result.params['N'].value, 3)) + '$\pm$' + str(
            ...
            plt.ylabel('Residual data')
            plt.grid(True)
            plt.savefig('IMAGE_' + LABEL + '_' + LAB + '_all_multi_' + meto + '.png', bbox_inches='tight')
            barra.update(7)
        ...
        # ________________________________________________________________________________________
        model = lmfit.Model(func_simple_DOP,
                            independent_vars=['theta_field', 'phi_field', 'theta_lua', 'phi_lua', 'theta_sol',
                                              'phi_sol',
                                              'alb', 'seeing', 'banda', 'wave_par'])
        model.set_param_hint('L', min=0, max=0.1)
        model.set_param_hint('k', min=3, max=20)
        model.set_param_hint('p', min=0.1, max=1)
        model.set_param_hint('N', min=3, max=20)
        p = model.make_params(L=np.random.rand(), k=np.random.rand(), p=np.random.rand(), N=np.random.rand())
        result_simple = model.fit(data=POL_OBS, params=p, theta_field=C1field, phi_field=C2field, theta_lua=C1lua,
                                  phi_lua=C2lua, theta_sol=C1sol, phi_sol=C2sol, alb=ALBEDO, seeing=SEEING,
                                  wave_par=WAV, weights=errPOL_OBS, method=meto)

        results_simple = [result_simple.params['L'].value, result_simple.params['N'].value,
                          result_simple.params['k'].value,
                          result_simple.params['p'].value]  # , result.params['N'].value]
        results_simple = np.asarray(results_simple)

        # data_parameters.append({'L': result_simple.params['L'].value, '$k_{1}$': result_simple.params['k'].value,'$k_{2}$': result_simple.params['p'].value,'N': result_simple.params['N'].value, 'c': result_simple.params['c'].value,'chi-square': result_simple.chisqr}, ignore_index=True)
        ...
        y1 = func_simple_DOP(theta_field=C1field, phi_field=C2field, theta_lua=C1lua, phi_lua=C2lua, theta_sol=C1sol,
                             phi_sol=C2sol, alb=ALBEDO, seeing=SEEING, wave_par=WAV, L=results_simple[0],
                             N=results_simple[1], k=results_simple[2], p=results_simple[3])
        ...
        if band is None or band is BAN:
            ...
            if isinstance(result_simple.params['p'].stderr, float):
                p_par = round(result_simple.params['p'].stderr, 3)
            else:
                p_par = result_simple.params['p'].stderr
            plt.ylim(0, 0.8)
            plt.ylabel('Polarization')
            label_text = 'fit parameters: L = ' + str(round(result_simple.params['L'].value, 3)) + '$\pm$' + str(
                L_par) + ',   $k_{1}$ = ' + str(round(result_simple.params['k'].value, 3)) + '$\pm$' + str(
                k_par) + ',   $k_{2}$ = ' + str(round(result_simple.params['p'].value, 3)) + '$\pm$' + str(
                p_par) + ',   $N$ = ' + str(round(result_simple.params['N'].value, 3)) + '$\pm$' + str(
                round(result_simple.params['N'].stderr, 3)) + '\n' + 'chi-square: ' + str(
                round(result_simple.chisqr, 5)) + ',  reduced chi-square: ' + str(
                round(result_simple.redchi, 5)) + '\n' + 'Bayesian Information Criterion: ' + str(
                round(result_simple.bic, 2))
            plt.annotate(label_text, xy=(0.1, 0.2), xycoords='axes fraction', xytext=(0.1, 1),
                         textcoords='axes fraction',
                         horizontalalignment='left', verticalalignment='center', bbox=dict(boxstyle="round", fc="w"))
            leg_text = 'simple fit with all corrections...\n \n$c_{B}$= ' + str(
                round(B_par[1], 3)) + '\n$c_{V}$= ' + str(round(V_par[1], 3)) + '\n$c_{R}$= ' + str(
                round(R_par[1], 3)) + '\n$c_{I}$= ' + str(round(I_par[1], 3))
            plt.annotate(leg_text, xy=(0.1, 0.2), xycoords='axes fraction', xytext=(1.05, 0.1),
                         textcoords='axes fraction',
    ...
    if answer == 'Yes':
        ...
        if answer_2 == 'Yes':
            plot = 'individual'
            answer_3 = str(input('Which one? Regular, Mix or Wave?\n'))
